...
def front_x(words):
    ...
    NotXlist = sorted([word for word in words if word[0] != 'x'])
    mylist= Xlist+NotXlist
    return mylist
...
def linear_merge(sorted1, sorted2):
    ...
    while True:
        if i == len(sorted1):
            merged_list= merged_list + sorted2[j:]
            break
        if j == len(sorted2):
            merged_list= merged_list + sorted1[i:]
            break
        if sorted1[i] <= sorted2[j]:
            merged_list.append(sorted1[i])
            i += 1
        else:
            merged_list.append(sorted2[j])
            j += 1
    return merged_list
